...
for iu=20,58,2 do begin
	...
	plot,feharr,sigmafeh,xtitle='[Fe/H]',ytitle='sigma FEH'
	outstr = {sn:iu,feh:feharr,sigmaage:sigmaage,sigmafeh:sigmafeh}
	;if iu eq 20 then finstr = outstr else finstr=[finstr,outstr]
	if iu eq 8 then finstr = outstr else finstr=[finstr,outstr]
endfor
