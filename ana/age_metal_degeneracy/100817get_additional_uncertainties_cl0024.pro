pro get_additional_uncertainties_cl0024,feh,age,sn,objname,ageupper,agelower,fehupper,fehlower
common degen, spec_arr,grid_feh,grid_age
   ...
   for i=0,ngals-1 do begin
       ...
       onesigmareg = where(chisqarr le min(chisqarr)+1.,conesigmareg)
       ageupper[i] = max(grid_age(onesigmareg))>age[i]
       agelower[i] = min(grid_age(onesigmareg))<age[i]
       fehupper[i] = max(grid_feh(onesigmareg))>feh[i]
       fehlower[i] = min(grid_feh(onesigmareg))<feh[i]
