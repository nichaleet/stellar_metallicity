pro test_degeneracy_formation_time3
...
;open or make the mock set of galaxies
	if file_test('mock_gal_degeneracy3.fits') eq 0 then make_mock_gal_degeneracy3 
	sdss = mrdfits('mock_gal_degeneracy3.fits',1)
	if file_test('mock_cl0024_degeneracy3.fits') eq 0 then make_mock_cl0024_degeneracy3
	sci = mrdfits('mock_cl0024_degeneracy3.fits',1)
...
;plotting
	...
	psname='Formation_Redshift_mock3.eps'
	device, filename = psname,xsize = 15,ysize = 10, $
		...
		oplot,marr,marr*mzrparam[1]+mzrparam[0]
	...
	psname='FEH_deviation_mock3.eps'
	Delta = '!9'+string("104B)+'!x'
	Delta = '!9'+string("104B)+'!x'
	device, filename = psname,xsize = 15,ysize = 10, $
			xoffset = 0,yoffset = 0,scale_factor = 1.0,/encapsulated,/color
		...
		print,A,B,sigma_a_b
		;oplot,[0,14],A+B*[0,14],color=cgcolor('darkgray'),thick=2,linestyle=2
