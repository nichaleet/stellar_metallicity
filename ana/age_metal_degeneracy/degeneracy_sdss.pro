pro degeneracy_sdss,sn=sn,redo=redo
;e.g. for i=20,58,2 do degeneracy_sdss,sn=i,/redo
common degen, spec_Arr, grid_feh, grid_age
	if n_Elements(redo) eq 1 then begin
		;make spectra with these grids
	;	grid_feh = findgen(100)/125.-0.6 ;range from [-0.6,0.192] dex
		grid_feh = findgen(120)/120.-0.8 ;range from [-0.8,0.192] dex ;for new_grid_spec_sdss.fits
		grid_age = findgen(126)/10.+0.5 ;range from [0.5,13] Gyr
		spec_arr = make_spec_sdss(grid_feh,grid_age)
	;	mwrfits,spec_arr,'grid_spec_sdss.fits',/silent,/create
		mwrfits,spec_arr,'new_grid_spec_sdss.fits',/silent,/create	
	endif else spec_arr = mrdfits('grid_spec_sdss.fits',1)
	stop
