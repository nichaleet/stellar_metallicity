...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	lin_mzr = linfit([sci(wpassivemem).logmstar,sdss.logmstar],[sci(wpassivemem).feh,sdss.feh],measure_errors=[sci(wpassivemem).feherr,sdss.feherr],sigma=lin_mzr_err,chisq=chilin)
	lin_mzr_sdss = linfit(sdss.logmstar,sdss.feh,measure_errors=sdss.feherr,sigma=lin_mzr_sdss_err,chisq=chilin_sdss)
	lin_mzr_cl = linfit(sci(wpassivemem).logmstar,sci(wpassivemem).feh,measure_errors=sci(wpassivemem).feherr,sigma=lin_mzr_cl_err,chisq=chilin_cl)
	...
	lin_mzr_sdss_test = linfit(sdss.logmstar-10,sdss.feh,measure_errors=sdss.feherr,sigma=lin_mzr_sdss_test_err,chisq=chilin_sdss_test,yfit=yfitsdss)
	lin_mzr_cl_test = linfit(sci(wpassivemem).logmstar-10,sci(wpassivemem).feh,measure_errors=sci(wpassivemem).feherr,sigma=lin_mzr_cl_test_err,chisq=chilin_cl_test,yfit=yfitcl)
        ;slope
        zslope = (lin_mzr_sdss_test(1)-lin_mzr_cl_test(1))/sqrt(lin_mzr_sdss_test_err(1)^2+lin_mzr_cl_test_err(1)^2)
	...
	masssdss_test = sdss.logmstar-10.
	masscl_test = sci(wpassivemem).logmstar-10.
        s_a1_a2 = sqrt(s2_sdss_cl*(1./nsdss+1./ncl+(mean(masssdss_test))^2/(variance(masssdss_test)*nsdss)+(mean(masscl_test))^2/(variance(masscl_test)*ncl)))
	...
	bestparam = intrinsic_scatter_linear([sci(wpassivemem).logmstar,sdss.logmstar],[sci(wpassivemem).feh,sdss.feh],[sci(wpassivemem).feherr,sdss.feherr],[lin_mzr,0.1])
	...
	bestparam_cl = intrinsic_scatter_linear(sci(wpassivemem).logmstar,sci(wpassivemem).feh,sci(wpassivemem).feherr,[lin_mzr_cl,0.1])
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
       psname='Cl0024_FeH_mass_slide4.eps'
        device, filename = psname,xsize = 15,ysize = 10, $
                xoffset = 0,yoffset = 0,scale_factor = 1.0,/encapsulated,/color
                ;make the outline of plots
                xrange=[8.9,11.5]
		...
		;oplot,[10.5,10.8],[-0.8,-0.8],color=fsc_color('maroon'),linestyle=2,thick=2
                ...
                al_legend,['Current work','Ma et al. 2016'],color='black',psym=[14,16],position=[10.6,-0.5],box=0,charsize=0.9,symsize=[1.3,1.3]               
                al_legend,['z~0','z=[0.3,0.4]','z=[0.8,1]'],psym=15,color=colorarr([1,2,4]),box=0,position=[10.6,-0.7]
                ;al_legend,['z=0','z=[0.1,0.2]','z=[0.3,0.4]','z=[0.6,0.7]','z=[0.8,1]'],psym=15,color=colorarr,box=0,position=[10.6,-0.6]
