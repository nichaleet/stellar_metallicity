...
for i=0,1 do begin
   ...
   masscolor=bytscl(mass,max=11,min=8.5)
   str = {mass:mass,feh:feh,feherr:feherr,zfit:zfit,objnames:objnames,goodfit:goodfit,afe:afe,afeupper:afeupper,afelower:afelower,goodsky:goodsky,age:age,ageerr:ageerr,masscolor:masscolor}
...
for i=1,0,-1 do begin
    if i eq 0 then str=goodstr else str=badstr
    if i eq 0 then symsize = 1.2 else symsize = 0.5
    normal = where(str.afe lt 0.4 and str.afe gt 0.,cnormal)
    upperlim = where(str.afe le 0.,cupperlim)
    lowerlim = where(str.afe ge 0.4,clowerlim)
    if i eq 1 then plot,str.age,str.afe,xtitle='Age(Gyr)',ytitle='[Alpha/Fe]',xrange=[1,11.5],psym=1,/nodata,yrange=[-0.1,0.5],position=[0.15,0.1,0.95,0.8]
    if cnormal gt 0 then begin
        nolowerlim = where(str.afelower(normal) lt -10,cnolowerlim)
        if cnolowerlim gt 0 then str.afelower(normal(nolowerlim)) = 2.*str.afe(normal(nolowerlim))-str.afeupper(normal(nolowerlim))
        noupperlim = where(str.afeupper(normal) gt 10,cnoupperlim)
    ...
    goodsky = where(str.goodsky eq 1,cgoodsky,complement=badsky,ncomplement=cbadsky)
    cgloadct,33
    if cgoodsky gt 0 then cgscatter2d,str.age(goodsky),str.afe(goodsky),color=str.masscolor(goodsky),psym=14,/overplot,symsize=symsize,/nodisplay,fit=0
    if cbadsky gt 0 then cgscatter2d,str.age(badsky),str.afe(badsky),color=str.masscolor(badsky),psym=16,/overplot,symsize=symsize,/nodisplay,fit=0
    plotsym,1,2,thick=2,color=fsc_color(icolor[i])
...
cglegend,psyms=[14,16],titles=['W/O Emission Lines','W/ Emission lines'],alignment=2,location=[1,0.05],length=0,charsize=0.8
cgloadct,33
cgcolorbar,divisions=4,minor=5,format='(F4.1)',range=[8.5,11],charsize=1.,title='Log(M/Msun)',tlocation='TOP'	
device,/close
