...
;read science
  ...
  scifits = dir+'field_highSN/sps_fit.fits.gz'
  clustername = 'fieldhighSN'
...
;start doing the chisq check
  ...
  for i=0,n_obj-1 do begin
     ...
     if min(science.dlam) gt 0.2 and max(science.dlam,/nan) lt 10.0 then begin
        ...
        specresfile = dir+'field_highSN/specres_poly.sav'
   ...
   ;make grids around the found age
    ...
    ;plot
    psname = outputdir+'/'+strtrim(string(i),2)+'_'+strtrim(science.objname,2)+'.eps'
    set_plot,'ps'
    !p.multi = [0,2,1]
