...
IF(ABS(NORM) GT ZERO) THEN BEGIN
    FOR JZ=IZ1,IZ2 DO BEGIN
        ...
        A[NSETP-1:M-1,JJ-1] += SM*A[NSETP-1:M-1,J-1]
    ENDFOR
    A[NSETP-1,J-1] = NORM
ENDIF
;   The following loop can be null.
...
FOR I=NSETP,1,-1 DO BEGIN
    IF (I NE NSETP) THEN Z[0:I-1] -= A[0:I-1,II-1]*Z[I]
    II = INDEX[I-1]
    Z[I-1] /= A[I-1,II-1]
ENDFOR
...
; Compute product of transormation and updated right-hand side.
    ...
    IF (ABS(NORM) GT ZERO) THEN BEGIN
        ...
        Z[NPP1-1:M-1] += SM*A[NPP1-1:M-1,J-1]
        A[NPP1-1,J-1] = NORM
    ENDIF

    IF (ABS(X[J-1]) GT ZERO) THEN $
        Z[0:NPP1-1] += A[0:NPP1-1,J-1]*X[J-1]
