...
;    if (psym[i] lt 0) || (psym[i] eq 10) then num = number > 2       ; TO SHOW CONNECTING LINE
    if (psym[i] lt 0) then num = number > 2       ; NL editted the line above
    if (psym[i] eq 10) then num = number > 1      ; NL editted the line above
...
;    if (num eq 1) && vertical then xp = x + xt/2      ; IF 1, CENTERED.
    ...
    if (p_sym ne 10) then begin
     ...
     ;This is when psym=10 (histogram). Originally, the bar length is too long. NL reduced to half.
     xt = 0.3*(xp[1]-xp[0]) ;NL added this line to make the text closer to symbol
     xp = [xp[0], xp[0], xp[0]+0.3*(xp[1]-xp[0]),xp[0]+0.3*(xp[1]-xp[0]), xp[0]] ;NL fixed the 3rd and 4th elements from xp[1] to current
     if (yp[0] eq yp[1]) then begin
      yp[0] -= (yspacing / 2.5);NL changed from /4 to /2.5
      yp[1] += (yspacing / 2.5)
     endif
