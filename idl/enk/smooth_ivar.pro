...
;
; PURPOSE:
;       Smooth a spectrum with a Gaussian kernal and inverse variance
;       weighting.
;
; CALLING SEQUENCE:
;       smooth_ivar, arrayin, ivarin, sigma, arrayout, ivarout
;
; INPUTS:
;       arrayin    input spectrum array
;       ivarin     input inverse variance array, same length as
;                  arrayin
;       sigma      sigma of the Gaussian kernel, in pixels
;
; OUTPUTS:
;       arrayout   smoothed spectrum array
;       ivarout    smoothed inverse variance
;-
...
pro smooth_ivar, arrayin, ivarin, sigma, arrayout, ivarout
     ...
     if n_elements(array) ne n_elements(ivar) then begin
         message, 'Wrong array dimensions in SMOOTH_IVAR.'
     endif
