pro consolidate_pk, mask, zspecpath=zspecpath, spec1dpath=spec1dpath, alldatapath=alldatapath
...
;          ;cont_err:0d
    ...
    ;calculate Ca EW
    for i=0,nzspec-1 do begin
        ca[i].sig_ca = ca_ew_test(zspec[i].spec1d_file, zspec[i].z, i, err=caerr, stop=long(zspec[i].objname) eq 4002236 ? 1 : 0)
    ...
    v_HB = 25.17
    feh_spec = -2.66 + 0.42 * (ca.sig_ca - 0.64 * (v_HB - (phot.vi0 + phot.i0)))
    ca.feh_spec = feh_spec
    ;print, feh_spec
