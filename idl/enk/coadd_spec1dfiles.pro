...
; CALLING SEQUENCE:
;  EITHER:
;    result = coadd_spec1dfiles(moogify, normalize=normalize, zero=zero,
;    inorm=inorm, ivartrim=ivartrim, weight=weight)
;
;  OR:
;    result = coadd_spec1dfiles(spec1dfiles, zarray,
;    normalize=normalize, zero=zero, inorm=inorm, ivartrim=ivartrim,
;    weight=weight)
; 
; INPUTS:
;    moogify -- an array of moogify structures
;
;  OR:
;    spec1dfiles -- a string array of pathnames to spec1d files
;    zarray -- an array of raw (observed) redshifts
;  
; OPTIONAL INPUTS:
;	
; KEYWORDS:
...
; enk 2009jun15, change to accept either moogify structures, or
; spec1dfiles and redshifts
;----------------------------------------------------------------------

function coadd_spec1dfiles, moogify, z, normalize=normalize, zeroweight=zeroweight, $
...
; set to unit average flux if desired
        if normalize OR useweight then begin
            whgood=where(ivar_in gt 0 and wave gt 8450 and wave lt 8700,ct)
