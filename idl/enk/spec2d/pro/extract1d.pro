...
;ENK begin
;Determine the x position of the trace based on a formula for
;differential atmospheric diffraction.
  ncols = n_elements(cwave)
  halfn = floor(ncols / 2.)
  diffrefrac = 206265. * (airindex_buie(double(cwave)*1d-4, 467., 0.0, 0.20) - airindex_buie(double(cwave[halfn])*1d-4, 467., 0.0, 0.20)) * tan((90. - sxpar(hdr, 'EL'))*!DTOR)   ;in arcsec
  diffrefrac_parallel = diffrefrac * cos((sxpar(hdr, 'PARANG') - sxpar(hdr, 'SLITPA'))*!DTOR)       ;parallel to slit (along spatial axis)
  diffrefrac_perpendicular = diffrefrac * sin((sxpar(hdr, 'PARANG') - sxpar(hdr, 'SLITPA'))*!DTOR)  ;perpendicular to slit (along wavelength axis)
  drpix_plel = diffrefrac_parallel * 8.52  ;8.52 pixels/arcsec
  drpix_perp = diffrefrac_perpendicular * 8.52  ;8.52 pixels/arcsec
  cwave_corr = cwave ;interpol(cwave, dindgen(npix), dindgen(npix) + drpix_perp)    ;SHOULD THIS BE + or - ?!!!
;ENK end
...
; find_object. 
      ...
      x = findgen(nrows)
      P = findgen(ncols, nrows)                                                   ;ENK - make the center of P a function of wavelength
      for i=0,ncols-1 do P[i,*] = exp( -(x - (x0+drpix_plel[i]))^2 / (2.*sigma^2) )    ;ENK - due to differential atmospheric refraction
...
; determine the shift needed to rectify the rows in wavelength.
        cshift = (i-pos)*dxdp    ;ENK - do not round this number
...
; etc.) are initially filled with zeros. 
        if optimal then begin
            spec_num = spec_num + $
              shift_interp(M[*,i] * P[*,i] * flux2d[*,i], cshift)   ;ENK - shift_interp instead of shift
            spec_denom = spec_denom + $
              shift_interp(M[*,i] * (P^2)[*,i], cshift) 
            var_num = var_num + $
              shift_interp(M[*,i] * P[*,i]^2 * var2d[*,i], cshift) 
            var_denom = var_denom + $
              shift_interp(M[*,i] * (P[*,i])^2, cshift) 
            sky_num = sky_num + $
              shift_interp(M[*,i] * (P[*,i]), cshift) 
        endif
        if horne then begin
            spec_num = spec_num + $
              shift_interp(M[*,i] * P[*,i] * flux2d[*,i] * ivar2d[*,i], cshift) 
            spec_denom = spec_denom + $
              shift_interp(M[*,i] * (P^2)[*,i] * ivar2d[*,i], cshift) 
            var_num = var_num + $
              shift_interp(M[*,i] * P[*,i]^2* ivar2d[*,i], cshift) 
            var_denom = var_denom + $
              shift_interp(M[*,i] * (P[*,i])^2 * ivar2d[*,i], cshift)
            sky_num = sky_num + $
              shift_interp(M[*,i] * (P[*,i]) * ivar2d[*,i], cshift)  
        endif
; determine the number of pixels at each wavelength which have a CR
; hit in them.
        crmask = crmask + shift_interp(slit.crmask[*,i] gt 0, cshift)
; take the minimum bitmask value (along the row) at each wavelength.
        bitmask = bitmask and shift_interp(slit.mask[*,i], cshift)
; also take the OR of the bitmask.
        ormask = ormask or shift_interp(slit.mask[*,i], cshift)
; determine the number of badpixels at each wavelength.
        nbadpix = nbadpix + shift_interp(slit.mask[*,i] ne 0b, cshift)
; take the OR of the infomask.
        infomask = infomask or shift_interp(slit.infomask[*,i], cshift)
    endfor
...
; construct the output structure and return it.
    ss1d =  {spec:spec, lambda:cwave_corr, ivar:ivar, crmask:crmask, $
...
; ENK: calculate an inverse varariance-weighted empirical 1-D spatial
; profile
        empirical = 1                                               ;ENK
        sprof1d = findgen(r2-r1+1)                                  ;ENK
        for i=r1,r2 do begin                                        ;ENK
            wint = where(interpolate[*,i] eq 0, cwint)              ;ENK
            if cwint eq 0 then begin                                ;ENK
                empirical = 0                                       ;ENK
                break                                               ;ENK
            endif                                                   ;ENK
            sprof1d[i-r1] = total(flux2d[wint,i]*ivar2d[wint,i]) $  ;ENK
                            /total(ivar2d[wint,i])                  ;ENK
        endfor                                                      ;ENK

        if empirical eq 0 then begin                                ;ENK
            message, 'Using Gaussian model spatial profile.', /info ;ENK
...
; add the flux in the spatial direction correcting for the shift.
        spec = spec + shift_interp(flux2d[*,i], cshift)
...
; with zeros and only filled it at the good pixels.
        var = var + shift_interp(var2d[*,i], cshift)
        count = count + shift_interp(var2d[*,i] gt 0, cshift)
; determine the number of pixels at each wavelength which have a CR
; hit in them.
        crmask = crmask + shift_interp(slit.crmask[*,i] gt 0, cshift)
; take the minimum bitmask value (along the row) at each wavelength.
