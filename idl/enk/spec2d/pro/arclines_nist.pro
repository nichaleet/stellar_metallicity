...
;  a = nist_read(path+'He.dat')
  ...
  ;arcfiles=['Cd.dat','Hg.dat','Ne.dat','Ar.dat','Kr.dat','Xe.dat','Zn.dat']
  ;arcfiles=['CdZnHgXeKrArNe_NIST_culled_by_Evan.dat']
  arcfiles=['NeArKrXe_NIST_culled_by_Evan.dat']

  narc=n_elements(arcfiles)

  for i=0,narc-1 do begin
      atmp=nist_read(path+arcfiles[i])
      ;atmp = atmp[where(strlowcase(strtrim(atmp.element, 2)) eq 'ne' or strlowcase(strtrim(atmp.element, 2)) eq 'ar' or strlowcase(strtrim(atmp.element, 2)) eq 'kr' or strlowcase(strtrim(atmp.element, 2)) eq 'xe')]
      ;if i eq 0 then maxlambda = max(atmp.lambda) else atmp = atmp[where(atmp.lambda gt maxlambda)]
...
;    n = 1.000275d
  ...
  listexists=findfile(deep_dir+'spec2d/etc/NeArKrXe_fewerblends.dat') NE ''
  
  if listexists eq 0 then begin
      openw,2,deep_dir+'spec2d/etc/NeArKrXe_fewerblends.dat'
