...
pro do_extract, files=files, nonlocal=nonlocal, $
                ...
                nsigma_boxcar=nsigma_boxcar, _extra=extra, calibsdir=calibsdir

if ~keyword_set(calibsdir) then calibsdir='.'
...
; must check the bin table file to see if the mask is a DEEP2 mask!
                  bintab = findfile(calibsdir+'/*.bintabs.fits*', count=bincnt)
                  if bincnt eq 0 then $
...
; tables.
              ...
              desipos = find_objpos(slitnum, npix_rows, fwhm=cat_width, $
                                    objnum=objnum, objtype=objkind, calibsdir=calibsdir)
...
; profile and to measure the fwhm of the profile.
              sprof = find_object(slit, slithdr=hdr, profivar=profivar, npix=npix, /rectify, /diffrefrac)   ;ENK - /rectify and /diffrefrac
...
; to the object position given by the design specifications.
                  ...
                  if mindiff le resolu or 1 then begin  ;ENK: added "or 1"; use for longslit only!
...
; make a post-script plot of the seeing difference distribution.
          ...
          legend_goddard, ['Amplitude = ' + string(a[0], format='(F6.2)'), $
...
; make plot.
  if cnt1 gt 50 and cnt2 gt 50 then begin
      ...
      legend_goddard, ['Solid Line = Blue Slits', 'Dashed Line = Red Slits']
      device, /close
      set_plot, 'x'
  endif
