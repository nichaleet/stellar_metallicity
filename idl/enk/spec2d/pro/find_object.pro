...
FUNCTION find_object, slitfile,  $
                      ...
                      rectify=rectify, diffrefrac=diffrefrac, slithdr=slithdr
...
;ENK begin -- account for differential atmospheric refraction and slit
;             tilt when finding object position and FWHM
  npix = (size(lambda))[1]
  nrows = (size(lambda))[2]
  halfp = floor(npix / 2.)
  halfc = floor(nrows / 2.)
  ...
  if keyword_set(rectify) then begin
      ...
      tiltx = (lambda[npix/2,nrows-1] - lambda[npix/2,0]) / nrows
      dxdp = tiltx/dldx ;slope of constant lambda/vert pix
      
      for i=0,nrows-1 do begin
          flux[*,i] = shift_interp(flux[*,i], double(i-halfc)*dxdp)
          ivar[*,i] = shift_interp(ivar[*,i], double(i-halfc)*dxdp)
          mask[*,i] = shift_interp(mask[*,i], double(i-halfc)*dxdp)
          infomask[*,i] = shift_interp(infomask[*,i], double(i-halfc)*dxdp)
      endfor
  endif
  if keyword_set(diffrefrac) then begin
      diffrefrac = 206265. * (airindex_buie(double(cwave)*1d-4, 467., 0.0, 0.20) - airindex_buie(double(cwave[halfp])*1d-4, 467., 0.0, 0.20)) * tan((90. - sxpar(slithdr, 'EL'))*!DTOR) * cos((sxpar(slithdr, 'PARANG') - sxpar(slithdr, 'SLITPA'))*!DTOR) ;in arcsec
      drpix = diffrefrac * 8.52  ;8.52 pixels/arcsec

      for i=0,npix-1 do begin
          flux[i,*] = shift_interp(reform(flux[i,*]), -1d * drpix[i])
          ivar[i,*] = shift_interp(reform(ivar[i,*]), -1d * drpix[i])
          mask[i,*] = shift_interp(reform(mask[i,*]), -1d * drpix[i])
          infomask[i,*] = shift_interp(reform(infomask[i,*]), -1d * drpix[i])
      endfor
  endif
;ENK end
...
;;; READ-IN THE SKY BAD PIXEL MASK skybpm. 
 ...
 if n_elements(mmm) eq 0 then mmm = 0
  ...
  bitmask = byte(mask)
  ivar = ivar 
  nearvig = (infomask AND 1b) eq 1b
  crmaskin = (infomask AND 8b) eq 8b
