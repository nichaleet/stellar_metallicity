pro read_planfile,  planfile, maskno, rawdatadir, outdatadir, $
        flatnames, arcnames, sciencenames, slitnums, nbright, ndim, nsimp, nlong, polyflag=polyflag,chips=chips, blueflats=blueflats, redflats=redflats, bluearc = bluearc, redarc = redarc, linelist = linelist, degree = degree
;+
; NAME:
;   read_planfile
;
...
;   blueflats  -- file names for blue-side flat                    ;ENK
;   redflats   -- file names for red-side flat                     ;ENK
...
;    substring = strsplit(line, /extract) ;split into two or more 
    ...
    case strmid(strupcase(substring[0]), 0, 3) of ;chars 0-2, upper case
      ...
      'BFL': if blueflats[0] eq '' then blueflats = $
                rawdatadir + strcompress(substring[1], /remove_all) else $
                blueflats= [blueflats, rawdatadir + $
                    strcompress(substring[1], /rem)]

      'RFL': if redflats[0] eq '' then redflats = $
                rawdatadir + strcompress(substring[1], /remove_all) else $
                redflats= [redflats, rawdatadir + $
                    strcompress(substring[1], /rem)]
...
; adding capability to use different arcs for red/blue sides
  if flatnames[0] eq '' then flatnames = blueflats          ;ENK
  if blueflats[0] eq '' then blueflats = flatnames          ;ENK
  if redflats[0] eq '' then redflats = flatnames            ;ENK
  ...
  if NOT quickfile then begin
     ...
     blueflats     = deimos_data  + blueflats               ;ENK
     redflats      = deimos_data  + redflats                ;ENK
