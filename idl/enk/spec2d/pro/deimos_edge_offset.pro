...
function deimos_edge_offset, xpos1, xpos2, ypos1, ypos2, specimage, $
                             specivar, sum=sum, longslits=longslits, shifterr=shifterr
...
;  vprint, 2,'Finding global flexure offset for this chip'
  ...
  for i=0, nslits-1 do begin 
     ...
     xrl = round(x0+2) > 0
     xru = round(x1-2) < (nx-1)
     ;EJT: added this for cases where slits are too small
     if xrl lt xru then begin
         vprint,2,'GOOD SLIT#',i,xrl,xru
         fakeprof[xrl:xru] = 1
     endif else begin
         vprint,2,'PROBLEM SLIT#',i,xrl,xru,'to',xrl-2,xru+2
         ;fakeprof[xrl-2:xru+2] = 1
     endelse
  endfor
  ...
  maxlag=5          ;ENK, 8-19-2010: changed 40 to 5
...
; do cross-correlation, subtract off BG to clean up 
  ...
  ;shiftfit=svdfit(lags[mintofit:maxtofit], $
  ...
  ;ET:debug - x-correlation problem
  ;dx0=0
  dx=dx+dx0
