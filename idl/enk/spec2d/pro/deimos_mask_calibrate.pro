...
; input the plan file for this mask
  read_planfile, planfile, maskname, rawdatadir, outdatadir, $
	     ...
	     chips=chiptext, bluearc = bluearc, redarc = redarc, $
                 linelist = linelist, $
                 blueflats=blueflats, redflats=redflats        ;ENK
...
; check to see if we're using different flats for the two sides        ;ENK
     diffflats = n_elements(blueflats) NE n_elements(redflats)         ;ENK
     if diffflats eq 0 then diffflats=total(blueflats eq redflats) $   ;ENK 
       ne n_elements(blueflats)                                        ;ENK
...
; limit to 3 flats.  When there are more, it is usually a mistake. 
  ;nflats = n_elements(flatnames)                                      ;commented out by ENK
  nblueflats = n_elements(blueflats)                                   ;ENK
  nredflats = n_elements(redflats)                                     ;ENK
  ;flatlimit=3-2*(quick gt 0)                                          ;commented out by ENK

  blueflatlimit = 10 - 9*(quick gt 0)                                   ;ENK
  if nblueflats GT blueflatlimit then begin 
     vprint,3,''
     vprint, 3, $
       string('You have', nblueflats, ' flats; I will use the last ',blueflatlimit, $
       format='(A,I2,A,I2)')
     blueflats = blueflats[nblueflats-blueflatlimit:nblueflats-1]
     nblueflats = n_elements(blueflats)
  endif 
  redflatlimit = 10 - 9*(quick gt 0)                                   ;ENK
  if nredflats GT redflatlimit then begin 
     vprint,3,''
     vprint, 3, $
       string('You have', nredflats, ' flats; I will use the last ',redflatlimit, $
       format='(A,I2,A,I2)')
     redflats = redflats[nredflats-redflatlimit:nredflats-1]
     nredflats = n_elements(redflats)
  endif 
...
; process optical model for this chip
     ...
     ;if strtrim(sxpar(arc_header, 'GRATENAM'), 2) eq '1200G' or strtrim(sxpar(arc_header, 'GRATENAM'), 2) eq '900ZD' then begin                                             ;ENK
         if (strtrim(sxpar(arc_header, 'GRATENAM'), 2) eq '1200G') then begin                                                        ;ENK
             case chipno of                                                                                                                                                 ;ENK
                 1: lambdashift = 20.                                                                                                                                       ;ENK
                 2: lambdashift = 20.                                                                                                                                       ;ENK
                 3: lambdashift = 20.                                                                                                                                       ;ENK
                 4: lambdashift = 20.                                                                                                                                       ;ENK
                 5: lambdashift = 20.                                                                                                                                       ;ENK
                 6: lambdashift = 20.                                                                                                                                       ;ENK
                 7: lambdashift = 20.                                                                                                                                       ;ENK
                 8: lambdashift = 20.                                                                                                                                       ;ENK
             endcase                                                                                                                                                        ;ENK
         endif else begin
             case chipno of                                                                                                                                                 ;ENK
                 1: lambdashift = 22.                                                                                                                                       ;ENK
                 2: lambdashift = 22.                                                                                                                                       ;ENK
                 3: lambdashift = 22.                                                                                                                                       ;ENK
                 4: lambdashift = 22.                                                                                                                                       ;ENK
                 5: lambdashift = 22.                                                                                                                                       ;ENK
                 6: lambdashift = 22.                                                                                                                                       ;ENK
                 7: lambdashift = 22.                                                                                                                                       ;ENK
                 8: lambdashift = 22.                                                                                                                                       ;ENK
             endcase                                                                                                                                                        ;ENK
         endelse
         wgoodslitmodel = where(model_lambda.xb gt 0 AND model_lambda.xt gt 0 AND model_lambda.lambda_y[0] gt 0)   ;ENK
         model_lambda[wgoodslitmodel].lambda_y[0] += lambdashift                 ;ENK added to account for difference
         model_lambda[wgoodslitmodel].lambda_y_top[0] += lambdashift             ;ENK in optical model from different
         model_lambda[wgoodslitmodel].lambda_y_bottom[0] += lambdashift          ;ENK slider position for grating
     ;endif                                                                       ;ENK
...
; process flats
    ...
    flatnamei = chipisred ? redflats : blueflats                                 ;ENK
    nflats = chipisred ? nredflats : nblueflats                                  ;ENK

    flatimage = deimos_read_chip(flatnamei[0], chipno, satmask=satmask, $        ;ENK
                                 quick=quick) 
...
; combine flats with avsigclip if there are more than 1
    if nflats gt 1 and quick le 0 then begin
       ...
       for j=0,nflats-1 do begin
          if j NE 0 then flatimage=deimos_read_chip(flatnamei[j], $              ;ENK
...
; count frames that went into pixel for inverse variance array
         ...
         for j=0,nflats-1 do begin
          if j NE 0 then flatimage=flatimage+deimos_read_chip(flatnamei[j], $    ;ENK
                                chipno, sat=satmask, quick=quick)
;          medimg = float(median(flatimage[sampleidx]))
;          sumcts = sumcts+medimg
;          flatarr[*, *, j]  = flatimage/medimg
;          flativar[*, *, j] = medimg^2/(flatimage > noisesq)
    ...
    if flatsig GT 1 AND nslits gt 0 then begin 
...
; tracing: 7 sec
      ...
      deimos_traceall, chipno, flatinterp, flativar, $
          ...
          indblu, ybin=ybin, pspath=pspath, badmatch=badmatch, $
        model=model_lambda;, /plot              ;ENK: add plot flag to diagnose slit tracing issues
...
; iterate once, just to be careful
    if quick le 0 then begin
       dxarc2 = deimos_edge_offset(xpos1+dxarc1, xpos2+dxarc1, $
                             ypos1, ypos2, arcimage, arcivar,/SUM, shifterr=shifterr)
       if shifterr lt dxarc2 then begin
           dxarc = dxarc1+dxarc2
           vprint,2, 'Pass 2:  dxarc: ', dxarc, ' [pix]'
       endif else begin
           dxarc = dxarc1
           vprint,2, 'Error toolarge: rejecting edge offset.'
       endelse
...
; Postscript Plot 3 - Arcs
    if keyword_set(pspath) then begin 
       ...
       nline = xpos1dim[0]                                                                ;ENK
       if nline eq 1 then begin                                                           ;ENK
           oplot, xpos1, ypos1,color=4                                                    ;ENK
           oplot, xpos2, ypos2,color=6                                                    ;ENK
...
;designated list, or if a wide slit.
        if slitwidth gt 1.95 then continue                    ;ENK -- skip alignment stars
...
; testing 2/28/02 - keep more slits
        ...
        slitisok=(highedge ge minok+25 AND lowedge LE maxok-25) $
          ...
          AND max(abs(x0)) lt 1.0e5 AND max(abs(x1)) lt 1.0e5     ;ENK

        if (highedge-lowedge) LT 15 then $
          vprint,2,string( 'Bad slit - too narrow!! ',bluslitno)
         if (highedge-lowedge) GT 1024 then $       ;ENK
...
;rectify arc 
      ...
      sizey = (size(rect_arc, /dimens))[1]
         ...
         minarc = 800
         if chipno le 4 and lambda_c lt 6000 then minarc = 25
         badslit = ((nearmaxinarc) lt minarc)
...
; Note that arcsat now carries vignetting info:
           ...
           if lambda_c gt 7000. AND grating ge 850 then begin
                ...
                deimos_fitwavelength, rect_arc, rect_arcsat+rect_nearvig, $
                 ...
                 polyflag = polyflag,polyx=polyx, s_coeff=s_coeff, dlam=dlam, $
                 dline=dline1, dlsig=dlsig1, wset=wset,sigma=sigma, $
                 blue_lax=(lambda_c lt 6000 and grating ge 900)   ;ENK
              endelse
