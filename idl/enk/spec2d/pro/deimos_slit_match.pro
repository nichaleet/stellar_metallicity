...
pro deimos_slit_match_sub, xblu, yblu, tset, ind, coeff, sigres=sigres, chipno=chipno    ;ENK: added chipno keyword
;----------------------------------------------------------------------
    if keyword_set(chipno) then begin                     ;ENK: added this if clause
...
; -------- compute tracesets for top (or bottom) of slits
  delvarx, xpos, ypos
  traceset2xy, tset, ypos, xpos
  xposdim = (size(xpos, /dimens))
  npix = xposdim[0]                                                       ;ENK
  if n_elements(xposdim) eq 1 then ntrace = 1 else ntrace = xposdim[1]    ;ENK
  ;ntrace = (size(xpos, /dimens))[1]
...
;  endif else begin
     onexy = transpose([[fltarr(n_elements(ind))+1], [xblu[ind]]])
     ;if ntrace eq 1 then coeff = [onexy, 0.] else $   ;ENK
     hogg_iter_linfit, onexy, found_x, weights, coeff
	coeff=[reform(coeff),0.]
;  endelse
...
; convert these to x-positions outside of deimos_slit_match
  ...
  ;stop
  if total(nmod) lt 7 then begin                      ;ENK: change 7 to 100 if things have gone badly
      usemodel=1                                      ;badly means that spec2d think that the slits overlap
      wh=where(model.xt NE model.xb)                  ;symptoms include more wavelength failures than usual
      switched=mean(model.xt[wh]-model.xb[wh]) lt 0   ;add plot flag to deimos_traceall in deimos_mask_calibrate (line 425) to be sure
...
; reversed top and bottom
      ...
      deimos_slit_match_sub, xblu1, yblu1, tset1, ind1, coeff1, sigres=sigres1, chipno=chipno
      deimos_slit_match_sub, xblu2, yblu2, tset2, ind2, coeff2, sigres=sigres2, chipno=chipno
  endelse
  badmatch=0
