...
; -------- Wavelength solution
     sigma =  deimos_arcfit(rect_arc, rect_arcsat, arcline_x, lamps, wset, $
 	  ...
 	  maxerr=0.15*(1200./float(grating))*(7800./float(lambda_c)), anamorph=anamorph, $
          ymid=ymid, chipno=chipno, blue_lax=blue_lax)  ;ENK: added chipno and blue_lax and lambda_c term
...
minpoly = 6                                                     ;ENK
if chipno le 4 and (keyword_set(blue_lax) or lambda_c lt 5800) then minpoly = 5       ;ENK

if n_elements(polyx) lt minpoly then badfit = 1                 ;ENK
