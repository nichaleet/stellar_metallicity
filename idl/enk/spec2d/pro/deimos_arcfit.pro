...
function deimos_arcfit, slitarc, arcsat, arcline_x, lamps_in, wset, $
        lamdif=lamdif, ncoeff = ncoeff,  slitwid=slitwid,  arcivar= arcivar, $
               ...
               dlam= dlam, qaparams=qaparams, dline=dline, dlsig=dlsig, $
                        ...
                        chipno=chipno, blue_lax=blue_lax         ;ENK
...
; -------- preliminaries
  ...
  fairlineerrorcut = 3.
  chicut = 20.
  if chipno le 4 and keyword_set(blue_lax) then begin
      maxerr *= 2.5
      fairlineerrorcut = 1.
      chicut = 100.
  endif
...
; -------- center up with peaklines and toss any faint lines
  ...
  arcline_xon = peaklines(slitarc[*, ymid], arcline_xon, $ 
                width=12*(slitwid/5.33 < 1.), nsmooth=round(slitwid), relax=blue_lax)
  if keyword_set(plot) then begin 
     atv, slitarc, max=10000, min=0
     atvplot, arcline_xon_save, replicate(ymid, nlamp), color=fsc_color('cyan'), psym=1
     atvplot, arcline_xon, replicate(ymid, nlamp), color=fsc_color('blue'), psym=4
  endif 
...
; median filter spectrum along central row
  ...
  slitarcsub = keyword_set(blue_lax) ? slitarc : slitarc-median(slitarc)   ;ENK
...
; in the detector, etc. at their center
  ...
  ;print, 'xerr2fit ', transpose(xerr2fit[ymid,*])           ;ENK
  ;print, 'maxerr ', maxerr/(1.+fairlineerrorcut*fairlines)  ;ENK
  ;print, 'chifit ', chifit                                  ;ENK  
  wgoodchi = where(chifit gt 0.1, cgoodchi)                 ;ENK
  if cgoodchi gt 0 then medchi = median(chifit[wgoodchi]) $ ;ENK
  else medchi = median(chifit)                              ;ENK
  weak = xerr2fit[ymid, *] GT maxerr/(1.+fairlineerrorcut*fairlines) $
    OR chifit gt chicut*medchi  ; This checks for CR-distorted lines   ;ENK
...
;make pretty plots
  if keyword_set(plot) then begin 
      for i=0, nlamp-1 do atvplot, x2[*, i], ycen[*, i], color=fsc_color('cyan')
      ;for i=0, nlamp-1 do atvplot, x3[*, i], ycen[*, i], color=fsc_color('blue')
  endif 

spline = 0  ; DPF - June 5 (DISABLED CODE REMOVED BY JAN 8/02)


polyflag = 1  ;MD June 25       ;ENK: removed comment from this line
...
; setup unchanging parameters
    xx = dindgen(npix)/(npix/2.) -1 ;need range -1,1 for orthogonal functions
    dx = double(2./npix)
    nrow2 = nrow/2
    ulim = 3*nrow/4
    llim = nrow/4
    dy = double(ulim-llim) ;number of rows

; fit lines, delete outliers in loop
    repeat begin
      degree = (6 < (nlamp - 2)) > 2                            
      t_order = 5 <  (nlamp -1) ;quadratic order if enough lines
...
; check to see if slopes are correct
      if keyword_set(plot) then begin
        ...
        for i=0, nlamp-1 do atvplot, [x3[nrow2,i]+dxx[i], $ 
           x3[nrow2, i]- dxx[i]], [0, nrow-1],  color=fsc_color('orange')
      endif
...
;      wave = slitarc*0. 
      ...
      bad = where(abs(dev) gt maxerr/(1.+fairlineerrorcut*fairlines), nbad) ;flag bad lines                ;ENK
...
;    print, 'finished lambda fits with ', nlamp, ' lines'
    ...
    if keyword_set(plot) then begin 
       for i=0, nlamp-1 do atvplot, x3[*, i], ycen[*, i], color=fsc_color('brown')
    endif
...
;      lamdif = (10.d^ytmp)-10.d^yfit
     ...
     if keyword_set(plot) then begin 
       for i=0, nlamp-1 do atvplot, x2[*, i], ycen[*, i], color=fsc_color('pink')
       xpred = traceset2pix(wset, ytmp[*, 0])
       for i=0, nlamp-1 do atvplot, xpred[i, *], ycen[*, i], color=fsc_color('red')
...
;xrange = [5500, 8500]
 ...
 endelse
  ...
  dline = dblarr(n_elements(lamps_in))
  dlsig = dblarr(n_elements(lamps_in))
  for i=0, nlamp-1 do begin 
...
; correct scatter for DOF!
 ...
 print
  ...
  ;if nlamp le 6 or sigma gt 0.2 then stop
