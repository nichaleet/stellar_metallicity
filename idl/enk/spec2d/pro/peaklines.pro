function peaklines, spec, yin, width=width, nsmooth=nsmooth, relax=relax  ;ENK - added relax
;+
; NAME:
;   peaklines
;
...
; local peaks, but not end points, then select maximum of these
       ...
       if keyword_set(relax) then begin                                      ;ENK
           if ((tempj[ii])[0] lt 1.3*min(tempj)) then yout[i] = -1.          ;ENK
