...
function deimos_rectify_slit, im, ivar, x0, x1, npad=npad, $
    ...
    ;wzero = where(x0 lt 0, czero)                  ;ENK: do not allow extraction beyond the edge of the chip
    ;if czero gt 0 then x0 -= min(x0)               ;ENK
    ;wmax = where(x1 gt 2047, cmax)                 ;ENK
    ;if cmax gt 0 then x1 -= max(x1)-2047.          ;ENK
