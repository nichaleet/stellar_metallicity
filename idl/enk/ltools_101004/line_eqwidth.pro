...
pro line_eqwidth_event,event
	...
	case uvalue of 
		...
		if event.press gt 0 then begin
			...
			plots,v(0),v(1),color=fsc_color("blue"),psym=2
			(*info).wpos(npos) = v(0)
			(*info).fpos(npos) = v(1)
			npos = npos + 1
		 	(*info).npos = npos
...
;
	...
	plot,wave,flux,xrange=[xmin,xmax],xstyle=1,yrange=[ymin,ymax], $
		color=fsc_color("white"),/nodata,xtitle='',ytitle='',title=''
;
; Fill line
;
	  	...
	  	polyfill,[wpos(0),wpos(0),wave(good),wpos(1),wpos(1)], $
			...
			color=fsc_color("red")
          	...
          	polyfill,[wpos(0),wpos(0),wpos(1),wpos(1)], $
                   	[fpos(0),fpos1(0),fpos1(1),fpos(1)],color=fsc_color("red")
  	    end
	end
;
; overplot data
;
	oplot,wave,flux,color=fsc_color("white")
	for i=0,info.npos-1 do plots,info.wpos(i),info.fpos(i),color=fsc_color("blue"),psym=2
	return
end
;============================================================= LINE_EQW_PS
;
	    ...
	    linterp,wave,flux,wpos,fpos1
	  	...
	  	polyfill,[wpos(0),wpos(0),wave(good),wpos(1),wpos(1)], $
			...
			color=fsc_color("green")
          	...
          	polyfill,[wpos(0),wpos(0),wpos(1),wpos(1)], $
                   	[fpos(0),fpos1(0),fpos1(1),fpos(1)],color=fsc_color("green")
  	    end
	end
;
; overplot data
