...
common xfuse_find_rooname_common,dir,root,names,dir_base,list_base, $
	...
	case uvalue of
		...
		list = findfile(dir+root+'*2ttagfcal.fit',count=n2)
		list = findfile(dir+root+'*2histfcal.fit',count=n)
		n2 = n2+n
		list = findfile(dir+root+'*3ttagfcal.fit',count=n3)
		list = findfile(dir+root+'*3histfcal.fit',count=n)
		n3 = n3+n
		list = findfile(dir+root+'*4ttagfcal.fit',count=n4)
		list = findfile(dir+root+'*4histfcal.fit',count=n)
		n4 = n+n4
...
;
	list  = findfile(directory+'*fcal.fit',count=n)
...
;
	...
	filename = findfile(st+'ttagfcal.fit',count=n)
	if n eq 0 then filename = findfile(st+'histfcal.fit',count=n)
	if n eq 0 then begin
...
;
	file = findfile(dir+rootname+segment+'ttagfraw.fit',count=n)
...
;
	file = findfile(dir+rootname+segment+'histfraw.fit',count=n)
	if n gt 0 then begin
		file=file(0)
