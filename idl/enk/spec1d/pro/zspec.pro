...
; Read in the 1d spectrum
  ...
  COMMON objectlist_COMMON, ols

  specfile = 'spec1d.'+results[slitcount].maskname+'.*' + $
             results[slitcount].objname+'.fits'
  if ols then $
    specname = findfile(djs_filepath(specfile, root_dir=fulldir[slitcount]), count=ct) $
  else  specname = findfile(djs_filepath(specfile, root_dir=fulldir), count=ct)
...
; Read in the 2d spectrum
  ...
  fname1 = 'slit.' + strcompress(results[slitcount].maskname, /remove_all) $
           ...
           + 'B.fits.gz'
  if ols then fname1 = concat_dir(fulldir[slitcount],fname1) $
  else fname1 = concat_dir(fulldir,fname1) 
  fname2 = 'slit.' + strcompress(results[slitcount].maskname, /remove_all) $
           + '.' + strcompress(results[slitcount].slitname, /remove_all) $
           + 'R.fits.gz'
  if ols then fname2 = concat_dir(fulldir[slitcount],fname2) $
  else fname2 = concat_dir(fulldir,fname2)
...
PRO zspec, mname, objlist=objlist, local_only=local_only, nodeep=nodeep, _extra=extra
; Main routine
...
; check if the objlist optional variable was passed....
  if n_elements(objlist) gt 0 then begin
      nobj = n_elements(objlist)
      fulldir = objlist.fulldir
      zbest = objlist.zbest
      zall = objlist.zall
      ...
      objnames = zbest.objname
      maskname = '0000'
      nonlocallist = lonarr(nslit)
      COMMON mobject_COMMON, mobject
      mobject = lonarr(nslit)
      ols = 1
  endif else ols = 0

  if ols eq 0 then begin
...
; check for each slit if there is more than 1 object in the slit.
  ...
  if ols then results.maskname = zbest.maskname
  FOR i=0, nslit-1 DO BEGIN
