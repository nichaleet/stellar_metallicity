...
; some slosh in dates
       ...
       ;    spec=(findfile(concat_dir(path,strcompress('spec1d*'+list[i].maskname+'*'+list[i].objname+'*',/rem))))[0]
       ;endif
       spec = strtrim(list[i].spec1d_file, 2)
    ...
    if strlen(spec) gt 1 then begin
     ...
     if inorm eq 0 then begin 
       ...
       ss1d=fill_gap(spec,/telluric,/silent,header=header) 
       wave=ss1d.lambda
       airtovac,wave
       ss1d.lambda=wave
     endif else ss1d=fill_gap(spec,/silent,header=header)
...
; need to do telluric corrections after adjusting ivar for best results
         ...
         remove_telluric, ss1d, airmass,/silent
         fix_response,ss1d
...
;before coadding, set regions of telluric absorption to have ivar=0
    ...
    endif else print,'missing '+string(list[i].maskname)+' '+string(list[i].objname)

   endfor
;
; now normalize 
     good = where(ivar gt 0, cgood)
