...
files=files[where(strpos(files,'.serendip') lt 0)]
...
for i=0,nfiles-1 do begin 
	...
	fitb[i,*]=skytweak_1d(s,degree=degree,template=stemp_flux,$
        ...
        calfile=findfile('calib*'+slitnum+'B*')
...
; get dlam here
	...
	fitr[i,*]=skytweak_1d(s,degree=degree,template=stemp_flux,$
        ...
        calfile=findfile('calib*'+slitnum+'R*')
        calstruct=mrdfits(calfile[0],1,/sil)
	...
	;if ct eq 0 then goodslit[i]=0 ;else $
		;goodslit[i]=widths[wh[0]] gt 0.65 AND $   ;ENK - changed 0.745 to 0.65
			;widths[wh[0]] lt 1.2 
        goodslit[i] = 1
    endfor
    ...
    baddlamb=abs(dlamb-djs_median(dlamb,width=(5 < nfiles),boundary='reflect')) $
      gt 3*sigdlamb

    baddlamr=abs(dlamr-djs_median(dlamr,width=(5 < nfiles),boundary='reflect')) $
      gt 3*sigdlamr
