...
function mpfitfun_moog_eval, p, dp, _EXTRA=extra
  ...
  if moogcontrevise eq 1 then begin
      ...
      moogspec = interp_moog7(p[0], p[1], p[2], p[3])
      moogspec = smooth_gauss_wrapper(mooglambda, moogspec, datalambda, dlam*p[4])
      ...
      for ccd=1,(hectospec eq 1 ? 1 : 2) do begin
          case ccd of
              1: begin
                  ...
                  wwhole2 = where(w2 lt nhalf)
              end
              2: begin
                  wwhole = where(w2 ge nhalf+5 and w2 lt n-5)
                  wwhole2 = where(w2 ge nhalf and w2 lt n)
              end
          endcase
          won = where(moogspec[wwhole] gt 0 and pixmask[wwhole] eq 1 and finite(contdivivar[wwhole]) and contdivivar[wwhole] gt 0.0, complement=woff, con)
          if con lt 300 then cont = dblarr(n_elements(wwhole))+1.0 else begin 
              won = wwhole[won]
              bkpt = slatec_splinefit(datalambda[won], contdiv[won]/moogspec[won], coeff, invvar=contdivivar[won], everyn=150, upper=3, lower=3, /silent)
              cont = slatec_bvalu(datalambda[wwhole2], bkpt, coeff)
          endelse
          case ccd of
              1: contb = cont
              2: contr = cont
      ...
      match, datalambda, x, w, wjunk
      speccont = speccont[w]
      if phot eq 1 then speccont = [1d, 1d, speccont]
      ymod = y / speccont
  endif else begin
      speccont = (y * 0d) + 1d
      ymod = y
...
function mpfitfun_moog, fcn, x, y, err, p, WEIGHTS=wts, FUNCTARGS=fa, $
