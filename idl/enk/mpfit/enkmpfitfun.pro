...
function mpfitfun_eval, p, dp, _EXTRA=extra
  ...
  if n_elements(wts) GT 0 then begin
      if f[0] gt 2 then wtsenk = wts*[1d, 1d, (1d - f[2:n_elements(f)-1])]/[1d, 1d, replicate(mean(1d - f[2:n_elements(f)-1]), n_elements(f)-2)] else wtsenk = wts*(1d - f)/mean(1d - f)
      result = (y-f)*wtsenk                      ;ENK - added (1d - f)
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] * wtsenk
      endif
  endif else if n_elements(err) GT 0 then begin
      if f[0] gt 2 then wtsenk = (1d / err)*[1d, 1d, (1d - f[2:n_elements(f)-1])]/[1d, 1d, replicate(mean(1d - f[2:n_elements(f)-1]), n_elements(f)-2)] else wtsenk = (1d / err)*(1d - f)/mean(1d - f)
      result = (y-f)*wtsenk                      ;ENK - added (1d - f)
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] * wtsenk
      endif
...
function enkmpfitfun, fcn, x, y, err, p, WEIGHTS=wts, FUNCTARGS=fa, $
