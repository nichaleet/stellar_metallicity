...
function mpfitfun_moog_eval, p, dp, _EXTRA=extra
  ...
  common chi2common, xch, ych, dych, mpspeccont, speccont

  np = n_elements(p)
  nf = n_elements(f)

  if moogcontrevise eq 1 then begin
      w2 = lindgen(n_elements(datalambda))

      moogspec_ch = interp_moog_ch(p[0], p[1], p[2], p[3], p[4])
      moogspec_ch = smooth_gauss_wrapper(mooglambda_ch, moogspec_ch, xch, dlam*p[5])

      won = where(moogspec_ch gt 0 and finite(dych) and dych gt 0.0, complement=woff, con)
      if con lt 100 then mpspeccont = dblarr(n_elements(xch))+1.0 else begin 
          wc = won
          for ii=0,9 do begin
              if 1 then begin
                  bkpt = slatec_splinefit(xch[wc], ych[wc]/moogspec_ch[wc], coeff, invvar=(dych[wc]/moogspec_ch[wc])^(-2.0), bkspace=150, upper=1, lower=1, /silent)
                  mpspeccont = slatec_bvalu(xch, bkpt, coeff)
              endif else begin
                  pcoeff = poly_fit(xch[wc], ych[wc]/moogspec_ch[wc], 2, measure_errors=dych[wc]/moogspec_ch[wc])
                  mpspeccont = poly(xch, pcoeff)
              endelse
              wcold = wc
              wc = won[where(abs(ych[won]/moogspec_ch[won] - mpspeccont[won])/(dych[won]/moogspec_ch[won]) lt 200, con)]
              if con lt 100 then break
              if array_equal(wc, wcold) then break
          endfor
      endelse

      match, xch, x, wmask, wjunk
      mpspeccont = mpspeccont[wmask] ;+ 0.05
      if phot eq 1 then mpspeccont = [1d, 1d, mpspeccont]
      ymod = y / mpspeccont
  endif else begin
      mpspeccont = (y * 0d) + 1d
      ymod = y
  ...
  ;constant = 1.0
  ;exponent = 0.0
  ;ftmp = phot eq 1 ? f[2:n_elements(f)-1] : f
  ;wttmp = (constant-ftmp)^exponent
  ;wttmp /= sqrt(mean(wttmp^2.0))
  ;newwt = phot eq 1 ? [1d, 1d, wttmp] : wttmp
  ;newwt = f*0.0d + 1.0d

  if 1 then begin
      plot, xch, ych/speccont
      oplot, x, f, color=fsc_color('red')
      wait, 0.5
  endif

  ;; Compute the deviates, applying either errors or weights
  if n_elements(wts) GT 0 then begin
      ;wtsmod = wts * mpspeccont * newwt
      wtsmod = wts * mpspeccont
      result = (ymod-f)*wtsmod
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0L, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] * wtsmod
      endif
  endif else if n_elements(err) GT 0 then begin
      ;wtsmod = (mpspeccont*newwt) / err
      wtsmod = mpspeccont / err
      result = (ymod-f)*wtsmod
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0L, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] * wtsmod
      endif
...
pro mpfitfun_moog_cash, resid, dresid
  COMPILE_OPT strictarr
  common mpfitfun_common, fcn, x, y, err, wts, f, fcnargs
...
function mpfitfun_moog_ch, fcn, x, y, err, p, WEIGHTS=wts, FUNCTARGS=fa, $
  ...
  if n_params() EQ 0 then begin
      message, "USAGE: PARMS = MPFITFUN_MOOG('MYFUNCT', X, Y, ERR, "+ $
        "START_PARAMS, ... )", /info
      return, !values.d_nan
  endif
  ...
  result = mpfit('mpfitfun_moog_eval', p, SCALE_FCN=scalfcn, $
