...
function interp_moog6, teff, logg, feh, alpha, mooglambda=mooglambda, fullres=fullres
...
;    endif
;
    mooglambda = read_moog_lambda6(n=nmoog, fullres=fullres)
        ...
        for j=0,ilogg-1 do begin
            for m=0,ifeh-1 do begin
                for n=0,ialpha-1 do begin
                    moogspeci = read_moog_bin6(tteff[i], tlogg[j], tfeh[m], talpha[n], n=nmoog, status=status, fullres=fullres)
...
;            moogspec += dteff[ar[0]]*dlogg[ar[1]]*dfeh[ar[2]]*dalpha[ar[3]]*moogspectot
;        endfor
;    endif
