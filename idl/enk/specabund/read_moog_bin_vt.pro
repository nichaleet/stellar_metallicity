function read_moog_bin, teff, logg, feh, alphafe, vt, n=n, status=status, fullres=fullres
    path = getenv('ahome')+'grid2/'+(keyword_set(fullres) ? 'synths/' : 'bin/')
    ...
    bingzfile = path+string(teff, gsign, abs(logg), fsign, abs(feh), asign, abs(alphafe), vt, format='("t",I04,"g",A1,I02,"f",A1,I02,"a",A1,I02,"v",I02,".bin.gz")')
