function tweak_wavelength, science, maskwhole
    if science.teff lt 3500 or science.teff gt 8000 or science.logg lt 0.0 or (science.logg lt 0.5 and science.teff gt 6800) or science.logg gt 5.0 or science.feh lt -4.0 or science.feh gt 0.0 or science.alphafe lt -0.8 or science.alphafe gt 1.2 then begin
    ...
    moogspec = interp_moog7(science.teff, science.logg, science.feh, science.alphafe)

    degree = 5
    errflag = 0

    nhalf = fix((n_elements(science.lambda)) / 2.)
    nwhole = n_elements(science.lambda)
    for ccd=1,2 do begin
        case ccd of
            1: begin
                lambda = science.lambda[0:nhalf-1]
                contdiv = science.contdiv[0:nhalf-1]
                contdivivar = science.contdivivar[0:nhalf-1]
                mask = maskwhole[0:nhalf-1]
            end
            2:  begin
                lambda = science.lambda[nhalf:nwhole-1]
                contdiv = science.contdiv[nhalf:nwhole-1]
                contdivivar = science.contdivivar[nhalf:nwhole-1]
                mask = maskwhole[nhalf:nwhole-1]
            end
        endcase
        ...
        temp_flux = moogspec
        temp_wave = mooglambda * (1d + science.zrest)
...
;----DEIMOS resolution
        ...
        whgood = where(finite(contdivivar) and contdivivar gt 0 and mask eq 1, goodct)
...
;---interpolate observed spectrum onto grid
        ...
        if ct gt 0 then fullmask = floor(interpol(mask[iwave], lambda[iwave], fullwave)) else fullmask = 0
...
;---march across spectrum in 100A windows, overlapping by 50A
                ...
                iwin = where((fullwave ge minwave) and (fullwave le maxwave) and (fullmask eq 1), cwin)
...
; evaluate new 1-d fit
            ...
            case ccd of
                1: begin
                    new_wave2_blue = new_wave2
                end
                2: begin
                    new_wave2_red = new_wave2
                end
            endcase
        endif else begin
    ...
    return, [new_wave2_blue, new_wave2_red]
end
