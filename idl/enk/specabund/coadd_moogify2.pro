...
function coadd_moogify2, struct, normalize=normalize, zeroweight=zeroweight, $
...
; some slosh in dates
       spec_in = struct[i].spec
       lambda_in = struct[i].lambda
       ivar_in = struct[i].ivar
...
; set to unit average flux if desired
     if normalize OR useweight then begin
         whgood=where(ivar_in gt 0 and finite(spec_in) and finite(ivar_in),ct)
         if ct gt 1 then norm=median(spec_in[whgood]) else norm=-1.E10
         if norm le 0 then ivar_in=ivar_in*0.
         if norm eq 0 then norm=-1.E10
