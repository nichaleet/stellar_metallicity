...
PRO zspec_selectz, event
  ...
  COMMON results_COMMON, results, slitcount, q_index
  COMMON oned_data_COMMON, specdata, smoothspec1, smoothspec2, synspec
  COMMON zspec_postagestamps_COMMON, show_stamps, ps_phot, ds9ps
  widget_control, event.top, get_uvalue=state
...
;  results[slitcount].zresult_all[event.index]
  ...
  ;if show_stamps eq -1 then zspec_showstamps
...
PRO zspec_showstamps
  	COMMON results_COMMON, results, slitcount, q_index
	COMMON zspec_postagestamps_COMMON, show_stamps, ps_phot, ds9ps

	r = where(ps_phot.objno eq results[slitcount].objname, count)
	ds9ps->delete_all_frames
	if count ne 1 then begin
		message, 'There is no corresponding image for this slit.', /info
		return
	endif
	ds9_stamp, ps_phot[r], ds9=ds9ps, mask=results[slitcount].maskname
END


PRO zspec_show_postagestamps, event
	COMMON zspec_postagestamps_COMMON, show_stamps, ps_phot, ds9ps

	show_stamps = not show_stamps
END
...
PRO zspec_update_atv, state
  ...
  atv, [[flux_red],[flux_blue]], _extra=extra, min=-25, max=100
...
;  Carries out all the initialisation of the widgets etc.
  ...
  menu2 = widget_button(menu, value='View', /menu, xsize=200)
	...
	if obj_Valid(ds9ps) then begin
    button = widget_button(menu2, value='Show Postage Stamps on Image', $
    				event_PRO='zspec_show_postagestamps')
	endif
...
;           redshift_text:redshift_text, $
	...
	slit_display_options = {fullslit: 1, untilt: 1}
...
; define the date to be used in the output file.
  ...
  dumpname = 'zspec.' + getenv('LOGNAME') + '.' + $
	...
	if not keyword_set(nodeep) then begin
		print, 'Starting Postage Stamp Service'
		; Deal with Postage stampe Startump
		;ds9ps = obj_new('ds9')
		show_stamps = 0
		ps_phot = getphot(strmid(string(mname), 0, 2))
	endif
