...
pro sps_iterproc, funcname, p, iter, fnorm, functargs=functargs, parinfo=pi, quiet=quiet, dof=dof
    ...
    if iter gt 1 then print, contiter, p[0], p[1], p[2],p[3],p[4], fnorm/dof, dof,abs(zdiff),abs(agediff),format='(I4,2X,D6.3,1X,D5.2,2X,D6.1,2x,D6.3,1X,D6.3,1X,D10.5,1X,I4,2X,D8.4,2X,D8.4)'
end
...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    pi[0].limits = [-.5,0.15]
    pi[1].limits = [3.,11]
    pi[2].limits = [0., 1000.0]
    pi[3].limits = [-0.05,0.05]+znow
    pi[4].limits = [0.0,0.4]
   ;;make the initial guesses unfix but within limits except redshift
    if science.feh eq -999 then pi.value = randomu(seed,5)*(pi.limits[1,*]-pi.limits[0,*])+pi.limits[0,*] else pi.value=[science.feh,science.age,science.vdisp,znow,science.alphafe]
    pi.value = randomu(seed,5)*(pi.limits[1,*]-pi.limits[0,*])+pi.limits[0,*]
    ;pi.value = [0.,3.3,600.,znow,0.2]
    pi[3].value = znow
    firstguess = pi.value    
    pi[0].limits = minmax(spsz)
    pi[0].limits =  [-0.1,max(spsz)]
    pi[1].limits = [min(spsage),alog10(galage(znow,1000))<max(spsage)]

    pi.step = double([0.1, 0.5, 25.0,0.0002,0.05])
    pi.parname = ['    Z', '  age', 'vdisp','redshift','[alpha/Fe]']
    pi.mpformat = ['(D6.3)', '(D5.2)', '(D6.1)','(D6.3)','(D5.3)']

    won = where(science.fitmask eq 1 and finite(science.contdiv) and finite(science.contdivivar) and science.contdivivar gt 0 and reallambda/(1.+znow) gt 3500. and reallambda/(1.+znow) lt 5400., con)
    ...
    print, '* * * * * * * * * * * * * * * * * * * *'
    print, '  i Z/Z_sun   age sigma_v  redshift [alpha/Fe]   chi^2  DOF'
    print, '--- ------- ----- ------- --------- ---------  -------- ----'
    ...
    while nloop lt maxnloop  do begin
        ...
        if status eq 0 then perror=pi.value*0.-99.
        ...
        afediff = (pi[4].value-pars[4])/pi[4].value
...
pro sps_fit::fitmg, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    wonmg = where(science.fitmgmask eq 1 and finite(science.contdiv) and finite(science.contdivivar) and science.contdivivar gt 0 and restlambda gt 5150., conmg)
    ...
    for i=0,nafearr-1 do begin
       spec = spsspec*response_fn[i].dspec
    ...
    ;;Plotting
    ;plot,datalam/(1.+znow),science.contdiv/science.spscont
    ;for i=0,nafearr-1 do if i mod 5 eq 0 then oplot,datalam[wonmg]/(1.+znow),spsspec[wonmg]*response_fn[i].dspec[wonmg],color=fsc_Color('red')
    ;oplot,datalam/(1.+znow),science.contdiv/science.spscont
    ;stop
    ...
    science.alphafe = bestafe[2]
    science.alphafeerr = (bestafe[3]-bestafe[1])/2.

    ;Printing
    print,'[Mg/Fe] results: Best, Median, Lower limit, Upper limit'
...
pro sps_fit::specres, science, qf=qf, goodoverride=goodoverride
  common mask_in, mask_in
  lambda = science.lambda
  if mask_in eq 'NGC6528' then science.dlam = 3.1/2.35 else science.dlam = science.lambda/1000.
end
