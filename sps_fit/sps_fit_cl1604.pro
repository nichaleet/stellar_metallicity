...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    pi.value = double([0.0, 3.0, 250.0,znow])
   ...
   ; pi[1].limits[0] = 1.0
    ...
    while abs(zdiff) gt 0.001 or abs(agediff) gt 0.001 or abs(vdispdiff) gt 0.001 or abs(redshfdiff) gt 0.001 and nloop le maxnloop do begin
        ...
        if curchisq gt 1.003*bestchisq then break
     endwhile
...
pro sps_fit::redraw
    ...
    case mode of
        ...
        4: begin        ;rest frame
            ...
            oplot, science.lambda/(1d +znow), science.contdiv, color=fsc_color('black') 
            nfull = n_elements(science.lambda)
            nhalf = fix(nfull/2)-1
            if science.wlfail_blue eq 1 then oplot, science.lambda[0:nhalf]/(1d +znow), science.contdiv[0:nhalf], color=fsc_color('darkgray') 
            if science.wlfail_red eq 1 then oplot, science.lambda[nhalf:nfull-1]/(1d +znow), science.contdiv[nhalf:nfull-1], color=fsc_color('darkgray') 
            n = n_elements(*self.tellstart)
            for i=0,n-1 do begin
...
pro sps_fit::getscience, files=files
    ...
    case 1 of
        ...
        strmid(mask_in, 0, 6) eq 'Cl1604' or strmid(mask_in, 0, 6) eq 'cl1604': begin
...
pro sps_fit::initialize_directory, directory=directory
    ...
    countfiles:
    files = file_search(directory, '*_1d.{fits,fits.gz}', count=c)
