...
pro fitspschoi_fix,science
  ...
  pi([2,3]).fixed = 1 ;fix velocity dispersion and redshift
  pi[3].value = science.zfit
  pi[2].value = science.vdisp
  ...
  while abs(zdiff) gt 0.001 or abs(agediff) gt 0.001 or abs(vdispdiff) gt 0.001 or abs(redshfdiff) gt 0.0001 do begin
     ...
     spsbestfit = get_sps_rest(restlambda, pi.value)
  ...
  science.covarfix = covar
  science.spscontfix = cont        
  science.spsspecfix = spsbestfit
  print, ' '
  done:
  science.fehfix = pi[0].value
  science.fehfixerr = perror[0]
  science.agefix = pi[1].value
  science.agefixerr = perror[1]
end
