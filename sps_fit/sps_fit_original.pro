...
pro sps_iterproc, funcname, p, iter, fnorm, functargs=functargs, parinfo=pi, quiet=quiet, dof=dof
    ...
    if iter gt 1 then  print, contiter, p[0], p[1], p[2], fnorm/dof, dof, format='(I3,2X,D6.3,1X,D5.2,2X,D6.1,1X,D8.3,1X,I4)'
end
...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    pi = replicate({value:0d, fixed:0, limited:[1,1], limits:[0.D,0.D], parname:'', mpprint:0, mpformat:'', step:0d, tied:''}, 3)
    pi.value = double([0.0, 5.0, 200.0])
    pi[0].limits = minmax(spsz)
    pi[1].limits = minmax(spsage)
    pi[2].limits = [0.0, 1000.0]
    pi.step = double([0.1, 0.5, 25.0])
    pi.parname = ['    Z', '  age', 'vdisp']
    pi.mpformat = ['(D6.3)', '(D5.2)', '(D6.1)']
    pi[1].limits[0] = 1.0

    if science.vdisp_smm gt 0 and science.vdisp_smm lt 900 then begin
        pi[2].fixed = 1
        pi[2].value = science.vdisp_smm
    endif
    ...
    print, '* * * * * * * * * * * * * * * * * * * *'
    print, '  i Z/Z_sun   age sigma_v    chi^2  DOF'
    print, '--- ------- ----- ------- -------- ----'

    while abs(zdiff) gt 0.001 or abs(agediff) gt 0.001 or abs(vdispdiff) gt 0.001 do begin
        contiter++
        pars = mpfitfun('get_sps', xmp, ymp, dymp, parinfo=pi, /nocatch, bestnorm=bestnorm, dof=dof, perror=perror, ftol=1d-10, gtol=1d-10, xtol=1d-10, covar=covar, nprint=1000, status=status, yfit=ympfit, iterproc='sps_iterproc')
        zdiff = (pi[0].value-pars[0])/pi[0].value
        agediff = (pi[1].value-pars[1])/pi[1].value
        vdispdiff = (pi[2].value-pars[2])/pi[2].value
        pi.value = pars

        spsbestfit = get_sps(restlambda, pi.value)
        ...
        science.spscont = cont ;nicha moved this line from below endwhile here
        science.spsspec = spsbestfit ;nicha moved this line from below endwhile here
    endwhile
    ...
    if pi[2].fixed eq 1 then begin
        science.vdisp = science.vdisp_smm
        science.vdisperr = -999.0
    endif else begin
...
pro sps_fit::newspec, increment=increment, noredraw=noredraw
    ...
    rgood = 0
    science = (*self.science)[self.i]
    bslitfile = file_dirname(science.spec1dfile, /mark_directory)+'slit.'+science.mask+'.'+string(science.slit, format='(I03)')+'B.fits.gz'
    rslitfile = file_dirname(science.spec1dfile, /mark_directory)+'slit.'+science.mask+'.'+string(science.slit, format='(I03)')+'R.fits.gz'
...
pro sps_fit::reprepare, nostatusbar=nostatusbar
    ...
    if finite(science.sn) then begin
       if science.sn gt 6. then science.good = 1 else science.good = 0 
    endif
    if update_phot eq 1 then begin
        common mask_in, mask_in
        case strmid(mask_in, 0, 4) of
            '0024': begin
...
pro sps_fit::continuum, science
    ...
    for ccd=1,ccd2 do begin
        ...
        case 1 of
            spline: begin
                bkpt = slatec_splinefit(lambda[won], spec[won], coeff, invvar=science.telldivivar[won], bkspace=330, upper=5, lower=1., /silent)
...
pro sps_fit::redraw
    ...
    case mode of
        ...
        1: begin                ;continuum fit
            ...
            oplot, science.lambda, science.telldiv, color=fsc_color('black')
        ...
        4: begin        ;rest frame
            ...
            oplot, science.lambda/(1d + science.zspec), science.contdiv, color=fsc_color('black')
...
pro sps_fit::initialize_directory, directory=directory
    common mask_in, mask_in
    newdirectory = '/scr2/nichal/workspace2/sps_fit/data/original_results/'+mask_in+'/'
...
pro sps_fit_original, mask
    common mask_in, mask_in
    mask_in = mask

    directory = '/scr2/nichal/keck/deimos/Cl0024MS0451/'+mask
