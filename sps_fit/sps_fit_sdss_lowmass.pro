...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
	...
	pi[0].limits = [-0.2,0.1]
	if science.haveemlines eq 1 then pi[1].limits=[min(spsage),3.0] else $
		pi[1].limits = [3.,(galage(znow,1000)/1.e9)<max(spsage)]	
   ...
   ;;make the initial guesses unfix but within limits except redshift
    ...
    while nloop lt maxnloop do begin
        contiter++
        dlam = dlam_all/2.  ;divide by 2 if the best fit vdisp is 0 km/s
        ...
        spsbestfitarr = get_sps_obs_sdss(reallambda, pars) 
...
pro sps_fit::fitfe, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    restlambda = reallambda / (1d + pi[3].value)
    rest = 1
    spsbestfitarr = get_sps_obs_sdss(reallambda, pi.value)
    rest = 0
...
pro sps_fit::fit_all
    ...
    curi = self.i
     for i=40,self.nspec-1  do begin
;    for i=0,39 do begin
        self.i = i
        self->default_range
        science = scienceall[self.i]
        if science.good eq 0 or i eq 117 then continue
        ;self->mask, science
        self->fit, science
...
pro sps_fit::mask, science, nomask=nomask, zfind=zfind, nozfind=nozfind, nmc=nmc
    ...
    if ~keyword_set(nomask) then begin
       ...
       ;;
        ...
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ;8)wavelength range
        w = where(science.lambda/(1.+science.zspec) lt 3650. or science.lambda/(1.+science.zspec) gt 6800.,c)
        if c gt 0 then mask[w]=0
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        n = n_elements(science.lambda)
...
pro sps_fit_sdss_lowmass, mask
    common mask_in, mask_in
    mask_in = mask
    directory = '/scr2/nichal/workspace2/lowmass/casjobs/'+mask
    if mask eq 'lowmass' then directory='/scr2/nichal/workspace2/lowmass/casjobs/spec'
    if mask eq 'lowmass_lowsn' then directory='/scr2/nichal/workspace2/lowmass/casjobs/spec_lowsn'
    if mask eq 'lowmass_lowsn_all' then directory='/scr2/nichal/workspace2/lowmass/casjobs/spec_lowsnall'
