...
pro mcmc_sps,x,y,yerr,nparam,nstep,p0,stepsizes,limits,paraname,returnvalues
  ...
  for ii=0UL,nstep-1 do begin
     ;if (ii mod 100.) eq 0. then print, 'Doing step',ii
  ...
  ;;plot chisq
  plot,chisqarr,xtitle='step number', ytitle='chisq',yrange=[min(chisqarr)-10.,max(chisqarr)+10.]
  ...
  for jj=0,nparam-1 do begin
     ...
     if jj ne nparam-1 then plot,xhist,yhist,ytitle='posterior PDF',xtitle=paraname(jj)
...
pro sps_fit::fit, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    ;contwon = where(science.contmask eq 1)
    ...
    print, '--- ------- ----- ------- ---------  -------- ---- -----  ------'
    print, 'initial guess is',pi.value
    ...
    uppervaluearr = fltarr(4,maxnloop)
    lowervaluearr = fltarr(4,maxnloop)

    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    widget_control, widget_info(self.base, find_by_uname='2d'), get_value=index
    wset, index

    ;;while abs(zdiff) gt 0.001 or abs(agediff) gt 0.001 or abs(vdispdiff) gt 0.001 or abs(redshfdiff) gt 0.001 and nloop lt maxnloop do begin
    while nloop lt maxnloop do begin
        ...
        ;;change initial guess to be within 1 sigma range of previous results
        ;if nloop gt 0 then begin
        ;   pi.value = randomu(seed,4)*reform((returnvalues[2,*]-returnvalues[0,*]))+reform(returnvalues[0,*])
        ;   pi[3].value = pars[3]
        ;   print, 'initial guess of mcmc: ',pi.value
        ;endif
        ;;fitting 4 parameters:Z,age,sigma,z
        rest = 0
        if nloop eq 0 then normalize = 1 else normalize = 0
        mcmc_sps,xmp,ymp,dymp,4,5000UL,pi.value,pi.step,pi.limits,pi.parname,returnvalues
        ...
        restlambda = reallambda/(1.+pi[3].value)
        ;;get the model
        ...
        ;;plot
        plot,xmp,ympold,xtitle='lambda',ytitle='ymp',xrange=[3500,5600]*(1.+pars[3])
        oplot,xmp,ymp,color=fsc_color('cyan')
        oplot,reallambda,spsbestfit,color=fsc_Color('red')

        plot,restlambda[won], science.contdiv[won] /spsbestfit[won]
        oplot,restlambda,cont,color=fsc_color('green')

        if nloop eq maxnloop-1 then print,'WARNING: MAX NLOOP REACHED!'

        dof = n_elements(ymp)-4.
        curchisq = bestnorm/dof
        print, '  i Z/Z_sun   age sigma_v  redhift    chi^2  DOF   ZDIFF  AGEDIFF'
        print,nloop,pars[0], pars[1], pars[2],pars[3],curchisq,dof,zdiff,agediff,format='(I4,2X,D6.3,1X,D5.2,2X,D6.1,2x,D6.3,1X,D10.6,1X,I4,2X,D8.4,2X,D8.4)'
        print, '* * * * * * * * * * * * * * * * * * * *'
        ;;keeping the values
        if savedata eq 1 and nloop mod 10 eq 0 then begin
           if nloop eq 0 then $
              str={won:won,lambda:science.lambda,contdiv:science.contdiv,contdivivar:science.contdivivar,fakecont:science.spscont,spsbestfit:spsbestfitarr[*,0],chisq:curchisq,param:pars,perror:perror,paramupper:reform(returnvalues[2,*]),paramlower:reform(returnvalues[0,*]),loop:nloop} else $
                 str={won:won,lambda:science.lambda,contdiv:science.contdiv,contdivivar:science.contdivivar,fakecont:science.spscont,spsbestfit:spsbestfit,chisq:curchisq,param:pars,perror:perror,paramupper:reform(returnvalues[2,*]),paramlower:reform(returnvalues[0,*]),loop:nloop}
           if nloop eq 0 then loopstr = [str] else loopstr=[loopstr,str]
           save,loopstr,firstguess,filename=self.directory+'/obj'+strtrim(string(self.i+1, format='(I3)'), 2)+'loopinfo.sav'
        endif
        ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        if curchisq lt bestchisq then begin
           ...
           bestcont = science.spscont
        endif
         chisqarr[nloop] = curchisq
         valuearr[*,nloop] = pars
         errorarr[*,nloop] = perror
         uppervaluearr[*,nloop] = reform(returnvalues[2,*])
         lowervaluearr[*,nloop] = reform(returnvalues[0,*])
        ;;if curchisq gt 1.03*bestchisq then break
        nloop +=1
     endwhile
    !p.multi = [0,1,1]
    if savedata eq 1 then begin
       str = {chisq:chisqarr,param:valuearr,perror:errorarr,upperparam:uppervaluearr,lowerparam:lowervaluearr,paraname:['Z','age','sigmav','redshift'],objname:strtrim(science.objname, 2),dof:dof}
       save,str,filename=self.directory+'/fitdata_'+strtrim(string(self.i+1, format='(I3)'), 2)+'.sav'
    endif
    print,agediff,zdiff,format='("--- ------- ----- ------- ---------  -------- ----",D9.6,2X,D9.6)'                            
    ;check if the last chisq is the best chisq
    if curchisq gt bestchisq then begin
    ...
    science.nloop = nloop
    science.spsspec = spsbestfitarr[*,0]     ;contdiv
    science.spsspecfull = spsbestfitarr[*,1] ;full model
    science.spscontfull = spsbestfitarr[*,2] ;continuum of the model
    print, ' '
    ...
    ;calculate chisq and new signal to noise
    if science.feh ne -999 then begin
       science.chisq = curchisq
       ;;signal to noise
       contmask = science.contmask
...
pro sps_fit::fitmg, science, noredraw=noredraw, nostatusbar=nostatusbar
    ...
    rest=1
    spsbestfitarr = get_sps_obs(reallambda, pi.value)
    rest=0
...
pro sps_fit::reprepare, nostatusbar=nostatusbar
    ...
    if update_phot eq 1 then begin
        ...
        case 1 of
            ...
            strmid(mask_in, 0, 4) eq '0451' or strmid(mask_in, 0, 4) eq 'mock': begin
...
pro sps_fit::specres_mask, directory
    sps_fit = mrdfits(directory+'/sps_fit_mcmc.fits.gz', 1, /silent)
...
pro sps_fit::mask, science, nomask=nomask, zfind=zfind, nozfind=nozfind, nmc=nmc
    ...
    if ~keyword_set(nomask) then begin
       ...
       ;1) remove emission lines
        ...
        ;3) where it peaks up greater than 4 sigmas
        ;w = where((science.contdiv-science.continuum) gt 4.*science.contdivivar^(-0.5), cw)
        ;if cw gt 0 then mask[w] = 0
        ...
        ;thresh_first = 2.5*stdev(first)
        ;thresh_second = 1.*stdev(second)
        ;skyres = where(abs(first) gt thresh_first and abs(second) lt thresh_second,cskyres)
        ;for i=0,cskyres-1 do begin
        ;   lambnow = science.lambda[skyres[i]]
        ;   w = where(science.lambda gt lambnow-1 and science.lambda lt lambnow+1,cw)
        ;   if cw gt 0 then mask[w]=0
        ;endfor
    ...
    case mode of
     ...
     ;       if cstart eq 0 or cend eq 0 then message, 'There are no continuum regions.'
            ...
            if science.feh lt 3. then begin
               normfactor =median(science.spsspecfull(where(finite(science.spsspecfull))))
            ...
            if science.fe lt 3. then begin
               normfactor = median(science.spsspecfullfe(where(finite(science.spsspecfullfe))))
            ...
            if science.mg lt 3. then begin
               normfactor = median(science.spsspecfullmg(where(finite(science.spsspecfullmg))))
            ...
            plot, science.lambda, science.telldiv/median(science.telldiv), xrange=self.lambdalim * (1d + science.zspec), yrange=self.ylim, xstyle=1, ystyle=1, background=fsc_color('white'), color=fsc_color('black'), xtitle='!6observed wavelength (!sA!r!u!9 %!6!n)!3', ytitle='!6flux (e!E-!N/hr)!3', /nodata, /noerase
        ...
        4: begin                ;;rest frame
           ...
           wstart = where(t eq 1, cstart)+1
            ...
            ;;plot the model from fe only
            if science.fe gt -10 then begin
               oplot, science.lambda/(1d + science.zspec), science.spsspecfullfe, color=fsc_color('darkgreen')
            ...
            if science.mg gt -10 then begin
               oplot, science.lambda/(1d + science.zspec), science.spsspecfullmg, color=fsc_color('blue')
...
pro sps_fit::getscience, files=files
    ...
    sciencefits = self.directory+(self.lowsn eq 1 ? 'sps_fit_mcmc_lowsn.fits.gz' : 'sps_fit_mcmc.fits.gz')
...
pro sps_fit::writescience
    ...
    scienceall = *self.science
    sciencefits = self.directory+(self.lowsn eq 1 ? 'sps_fit_mcmc_lowsn.fits' : 'sps_fit_mcmc.fits')
...
pro sps_fit::initialize_directory, directory=directory
    ...
    sciencefits = newdirectory+(self.lowsn eq 1 ? 'sps_fit_mcmc_lowsn.fits.gz' : 'sps_fit_mcmc.fits.gz')
    if c eq 0 then begin
       files = file_search(directory+'*/spec1d*.{fits,fits.gz}', count=c)
    endif
...
pro sps_fit_mcmc, mask
    ...
    if strlowcase(strmid(mask,0,3)) eq 'all' then directory = '/scr2/nichal/keck/deimos/Cl0024MS0451/nicha_reduced/'
